...
func (c *BitqueryClient) UpdateAndGetTransfers(tokenAddress string) (string, error) {
	...
	for addr, amount := range holderBalances {
		if rec, exists := unionMap[addr]; exists {
			rec.Amount = amount // Update the amount for existing holders
			unionMap[addr] = rec
		} /*else {
			...
			unionMap[addr] = HolderInfo{
				Holder: addr,
				Amount: amount,
				Time:   time.Now().Format(time.RFC3339), // Use current time as we don't know when they acquired tokens
				Price:  unionMap[addr].Price,            // We don't know the price they acquired at
			}
		}*/
	}
